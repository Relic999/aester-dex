...
logErrorWithTimestamp('Hunter: Failed to initialize symbol precision manager:', error);
    ...
    // In paper mode, either simulate or use real liquidations based on config
    if (this.config.global.paperMode) {
      if (this.config.global.useRealLiquidations) {
        logWithTimestamp('Hunter: Running in PAPER MODE with REAL liquidation data from exchange');
        this.connectWebSocket();
      } else {
